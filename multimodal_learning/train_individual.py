...
training_params.__new__.__defaults__ = (2e4, 16, 10, 600, 100) 
tps = training_params()
...
def train(args):
    ...
    with g.as_default():
        ...
        while not sv.should_stop():
            ...
            feed_dict = {train_ops['x']: x, train_ops['y']: y} #, train_ops['lr']: lr}

            (_, train_loss, train_y_) = s.run([train_ops['optimiser'], train_ops['loss_all'],
                                                             train_ops['y_']],
                                              feed_dict=feed_dict)

            print("Train: Loss= {:.6f} {:.6f}".format(
                train_loss, 1 - np.mean(metrics.dice(train_y_, y, num_classes))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_csv', default='train_CT_set1.csv')
    parser.add_argument('--val_csv', default='val_CT_set1.csv')

    parser.add_argument('--save_path', '-p', default='CT_set1')
