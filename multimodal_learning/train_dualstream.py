...
def validate(ops, session, supervisor, name, v_all=True):
    ...
    if v_all:
        num_validation_examples = len(ops['filenames2'])
    else:
        num_validation_examples = 4

    val_idx = range(num_validation_examples)

    for f in ops['filenames2'][val_idx]:

        # Read a validation image and label of arbitrary dimensions
        val_x, val_y = ops['read_func2']([f])

        pid = 'Subj.' + f[0].split('p/')[1][:2]
             
        y_prob = sliding_window_segmentation_inference(session, [ops['y_prob']], {ops['x']: val_x}, {ops['streamid']: 1}, batch_size=1)[0]
...
def create_ops(net, mode='train'):
    ...
    ops['streamid'] = tf.placeholder(tf.int32, shape=[], name='streamid_placeholder')    

    ops['net'] = net(ops['x'], ops['streamid'], is_training=is_training)
    ...
    if is_training:
        ...
        # Create a learning rate placeholder for scheduling and choose an optimisatio
        ops['global_step'] = tf.Variable(0, name='global_step', trainable=False)
        ops['optimiser'] = tf.train.AdamOptimizer(0.0001, epsilon=1e-5).minimize(ops['loss_all'],
...
def train(args):
    ...
    with g.as_default():
        ...
        net = DualStreamFCN_v2(num_classes,
        ...
        if args.run_validation:
            ...
            val_ops['filenames2'] = pd.read_csv(args.val_csv2, dtype=str).as_matrix()
            val_ops['reader2'] = reader.OrgansReader([None, None, None],
                                                       name='val_queue2')
            # Get the read function in mode is_training=False to prevent augmentation
            val_ops['read_func2'] = lambda x: val_ops['reader2']._read_sample(x, is_training=False)
            print('Loading validation file names from %s' % args.val_csv2)
        ...
        while not sv.should_stop():
            ...
            if  (step %2 == 0):
                ...
                feed_dict = {train_ops['x']: x, train_ops['y']: y, train_ops['streamid']: 0}
                ...
                feed_dict = {train_ops['x']: x, train_ops['y']: y, train_ops['streamid']: 1}
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Malibo-CT segmentation training')
    ...
    parser.add_argument('--val_csv', default='val_MR_set1.csv')
    parser.add_argument('--val_csv2', default='val_CT_set1.csv')
    parser.add_argument('--save_dscs', default='DSC_v2_set1.npy')

    parser.add_argument('--save_path', '-p', default='dualstream_v2_set1/')
